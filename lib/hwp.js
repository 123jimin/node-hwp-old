...
HWPDocument.prototype._body = function _body(callback){
	...
	this._sections(callback, body, this._doc.storage('BodyText'), 0, numSections);
};
...
HWPDocument.prototype._sections = function _sections(callback, body, storage, cid, mid){
	...
	getRawBuffer(this._hwp_meta, storage.stream('Section'+cid), function(buffer){
		var section = _this._section(buffer);
		section.attr.Id = cid;
		body.add(section);
...
// Get HML Section
HWPDocument.prototype._section = function _section(buffer){
