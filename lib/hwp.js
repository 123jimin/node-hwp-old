						...
						for(var j=0;j<record.child.length;j++){
							...
							switch(c.tag){
								...
								case HWPTAG.BORDER_FILL:
									...
									var borderFill = new hml.BORDERFILL();
									
									var leftBorder = borderFill.getChild('LEFTBORDER');
									var rightBorder = borderFill.getChild('RIGHTBORDER');
									var topBorder = borderFill.getChild('TOPBORDER');
									var bottomBorder = borderFill.getChild('BOTTOMBORDER');
									var diagonal = borderFill.getChild('DIAGONAL');
									
									borderFillList.add(borderFill);
									borderFill.attr.Id = borderFillID++;
									
									var prop = c.data.readUInt16LE(0);
									borderFill.attr.ThreeD = !!(prop&0x1);
									borderFill.attr.Shadow = !!(prop&0x2);
									borderFill.attr.Slash = ((prop>>2)&0x7);
									borderFill.attr.BackSlash = ((prop>>5)&0x7);
									
									leftBorder.attr.Type = hml.LineType1[c.data.readUInt8(2)];
									leftBorder.attr.Width = hml.LineWidth[c.data.readUInt8(3)];
									leftBorder.attr.Color = c.data.readUInt32LE(4);
									
									rightBorder.attr.Type = hml.LineType1[c.data.readUInt8(8)];
									rightBorder.attr.Width = hml.LineWidth[c.data.readUInt8(9)];
									rightBorder.attr.Color = c.data.readUInt32LE(10);
									
									topBorder.attr.Type = hml.LineType1[c.data.readUInt8(14)];
									topBorder.attr.Width = hml.LineWidth[c.data.readUInt8(15)];
									topBorder.attr.Color = c.data.readUInt32LE(16);
									
									bottomBorder.attr.Type = hml.LineType1[c.data.readUInt8(20)];
									bottomBorder.attr.Width = hml.LineWidth[c.data.readUInt8(21)];
									bottomBorder.attr.Color = c.data.readUInt32LE(22);
									
									diagonal.attr.Type = hml.LineType1[c.data.readUInt8(26)];
									diagonal.attr.Width = hml.LineWidth[c.data.readUInt8(27)];
									diagonal.attr.Color = c.data.readUInt32LE(28);
									break;
...
HWPDocument.prototype.getHML = function(){
	return this._hml;
};
...
exports.Convert = {
	'HTML': require('./converter/html.js').convert
};
